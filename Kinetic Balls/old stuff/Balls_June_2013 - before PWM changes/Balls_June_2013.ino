...
//Constants:
  ...
  const byte maxCycles = 3;            // How many loops to run through before an automatic recalibration  xxx normally 15 or more?  
...
/*
      ...
      case 2:   // random points program
        ...
        do {
		  ...
		  pauseTime(10); // temp override for debugging
   ...
   */    
      ...
      do {
		...
		//motorDirectionDown(1);			            // go down
		pwmMotor(1, 25,  5, 30);					// slow
		pwmMotor(1, 13, 13, 20);					// medium
		pwmMotor(1, 10, 50, 10);					// fast
		pwmMotor(1, 13, 13, 20);					// medium
		pwmMotor(1, 25,  5, 30);					// slow

		setRegisterPin((3*1), LOW );		        // OFF
		writeRegisters();

		pauseTime(5);

		motorDirectionUp(1);						// go up
		pwmMotor(1, 25,  5, 30);					// slow @ 25%
		pwmMotor(1, 13, 13, 20);					// medium @ 50%
		pwmMotor(1,  5, 25, 10);					// fast @ 75%
		pwmMotor(1,  0, 25, 150);					// full speed @ 100%
		pwmMotor(1,  5, 25, 10);					// fast
		pwmMotor(1, 13, 13, 20);					// medium
		pwmMotor(1, 25,  5, 30);					// slow
...
//------------------------------------
void pwmMotor(byte motor, byte offDuty, byte onDuty, byte loops){
