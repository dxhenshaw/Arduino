...
prog_char menuItem_7[] PROGMEM   = "Sleep";
prog_char menuItem_8[] PROGMEM   = "Recalibrate";
prog_char menuItem_9[] PROGMEM   = "Set sleep/wake time";
prog_char menuItem_10[] PROGMEM  = "Set Time";
prog_char menuItem_11[] PROGMEM  = "Stop";
prog_char menuItem_12[] PROGMEM  = "Set ceiling";
prog_char menuItem_13[] PROGMEM  = "Set floor";
prog_char menuItem_14[] PROGMEM  = "Rotate & push to set";  //status messages from this point on
prog_char menuItem_15[] PROGMEM  = "Recalibrating...";//not being shown xxx
prog_char menuItem_16[] PROGMEM  = "Move to ceiling...";
prog_char menuItem_17[] PROGMEM  = "Move to floor...";
prog_char menuItem_18[] PROGMEM  = "Sleeping";
prog_char menuItem_19[] PROGMEM  = "STOP. Push to start";
prog_char menuItem_20[] PROGMEM  = "Moving...";
prog_char menuItem_21[] PROGMEM  = "Moving down...";   //xxx not currently being used
prog_char menuItem_22[] PROGMEM  = "Menu: Turn dial+wait"; //xxx not currently being used
prog_char menuItem_23[] PROGMEM  = "Paused";
...
#define allBlinkMs 0x00  // This sketch sends to the I2C "broadcast" address of 0, so all BlinkMs on the I2C bus will respond
...
//Constants:
  ...
  const int minPauseTime = 20;              // in seconds = 2 minutes = 120 xxx
  const int maxPauseTime = 30;              // in seconds = 5 minutes = 300 xxx
  const int maxIndex = 250;
  const byte maxMenuItems = 13;          //how many menu items there are
  const int encoderGap = 2;             //size of gap from encoder before we pay attention to it
  const byte msgUseDial = 14;            //message pointers for menu follow...
  const byte msgRecalibrate = 15;  
  const byte msgCeiling = 16;
  const byte msgfloor = 17;
  const byte msgSleeping = 18;
  const byte msgStopped = 19;
  const byte msgMoving = 20;
  const byte msgDown = 21;
  const byte msgSeeMenu = 22;
  const byte msgPause = 23;
  const byte maxCycles = 5;            // How many loops to run through before an automatic recalibration  xxx normally 10 or more?
...
//Variables:
  ...
  int selectedMenuItem = 99;            //value of menu selection at the time the button was pushed
  int lastEncoderValue = 1;
  int ceilingIndex = 3;                //index for ceiling level
  int floorIndex = 60;                 //index for floor level
  ...
  byte wakeHour = 17;                 // time to wake up
  byte loopCounter = 0; 
...
if(digitalRead(encoderSwitchPin)){  // check if button has been pushed 
    switch (selectedMenuItem) {  // button is not being pushed, which means we can execute code based on it being pushed last time round this loop
      ...
      case 1:    // ladder program (up and down while running scripts)
          ...
          recalibrate();
          Serial.println("@pgm 1 down");
          do { // head down until units reach the floor
            targetIndex = programTargetIndex(motor[0][motorCurrentIndex], floorIndex); // find a random place to go between currentIndex of motor 0 [0][0] and floor
      ...
      case 3:  // 
        Serial.println("@pgm 3 - angles");
        recalibrate(); 
        loopCounter = 0; 

         do {      
          pointA = programTargetIndex( ceilingIndex, ((floorIndex - ceilingIndex) * 0.4) ); // PointA: find a random point between the ceiling and top 40% from the ceiling xxx need to find way to do lower 40%
        ...
        //Serial.println("Leaving Angles");
        break;

      case 4:  // 
       Serial.println("@pgm 4 - waves");
       recalibrate();
       ...
       pointA = calculateSinPosition(loopCounter, sineFrequency);  // pointA is the value for unit 10 xxx need to convert into range between ceiling and floor
       ...
       do{
         ...
         curtainsOut(100, white, black);
         BlinkM_fadeToRandomRGB(allBlinkMs, 0xff, 0xff, 0xff);      // set a random color
         programPauseTime = random(20, minPauseTime);       // in seconds - wait before moving
         pauseTime(programPauseTime); 
         ...
         loopCounter ++;
              if (loopCounter > (maxCycles * 6)) { // xxx check this size loop
              break; }
      ...
      case 5:  // 
        ...
        Serial.println("Leaving lines");
        break;
        
      case 6:  // Light show
        Serial.println("@pgm 6");
        
        if (doNotRecalibrate == false) { // first time here, so we do need to recalibrate this one time
          recalibrate(); // find a way to recalibrate only once so you can leave this program running for ever xxx
          ...
          gapA = ((pointB - pointA) / 15) ; // 
       
          motor[0][motorTargetIndex] = pointA;                  // top
          motor[1][motorTargetIndex] = pointA + (6 * gapA);
          motor[2][motorTargetIndex] = motor[1][motorTargetIndex] + (4 * gapA);
          motor[3][motorTargetIndex] = motor[2][motorTargetIndex] + (3 * gapA);        
          ...
          ballFadeTo(allBlinkMs, blue); } // end of doNotRecalibrate
     
       programPauseTime = random(10, minPauseTime);           // in seconds - wait before moving
       pauseTime(programPauseTime);                                // convert to milliseconds xxx add message on pause time
       i = random(0,11);  // pick a random light show from 0-10
       randomLightShow(i);       
      ...
      case 9:  // set sleep and wake time 
      ...
      case 13:  //Set floor
       ...
       do{
        ...
        floorIndex = targetIndex;              // new value for floor
        clearLine(2);
        clearLine(3);
        selectedMenuItem =99;  // prevent this case from being selected next time round
        break;
    ...
    if (sleepHour == RTC.getHours()) selectedMenuItem = 7;                         // go to sleep if it's sleeping hour
    if (autoRunFlag == true){  // we're in auto-run mode and need to cycle through each program in turn
      ...
      if (selectedMenuItem > maxMenuItems) selectedMenuItem = 1; } // next time round, the correct program will run
...
void randomLightShow(byte i) {
 ...
 switch (i) {  // 
        ...
        case 7:    // play a random script between script 10 and 16 inclusive
          BlinkM_playScript(allBlinkMs,(random(10,17)), 0, 0); //         
        break; 
...
void pauseTime(int k) { // pause for k seconds, including a countdown
  ...
  for (int i=k ; i > 0; i--) {
    ...
    if ((RTC.getMinutes() != prevMinutes) && (selectedMenuItem != 7)) {  // if one minute has passed and we're not in sleep mode (7)
       randomLightShow(random(0,11));  
       prevMinutes = RTC.getMinutes();
    }
...
for (int i=0 ; i < 10; i++) {
  ...
  if (motor[i][motorCurrentIndex] > motor[i][motorTargetIndex]) {  // if currentIndex > targetIndex, need to move up CW
    Serial.print("Up ");
    motorDirectionUp(i);    
    motor[i][motorDeltaMoves] = motor[i][motorCurrentIndex] - motor[i][motorTargetIndex] + 0;  //move this many steps: current - Target + 0
    ...
    motor[i][motorDeltaMoves] = motor[i][motorTargetIndex] - motor[i][motorCurrentIndex] - 1; //move this many steps: target - Current - 1
  }
}
...
for (int i=0 ; i < 10; i++) {            // figure out which motors need to have their pin enabled
  ...
  writeRegisters();                      // latch shift register. Motors spin from this point on!

 do {     
    lcd.setCursor(17,3);
    printPositionDigits(maximumDelta);                // update LCD with current step number
    maximumDelta --;
    ...
    for (byte l=3; l>0; l--) {            // check ball status 3 times per second
      ...
      for(byte i = 6; i < DATA_WIDTH; i++){  //loop thru each bit from bit positions 6 thru 15 (== units 0 thru 9)
        if(((pinValues165 >> i) & true) && (motorRecalibration[i-6] == false)) {       //if the bit equal to 1 (true) and recalibration flag is false, then
          motorRecalibration[i-6] = true;  //set this unit's recalibration flag to true                                            
          Serial.print(i-6);
          Serial.println(" -1");    //we are at position -1 and need to unwind until the switch turns off
          motorDirectionDown(i-6);          
          writeRegisters();                // latch shift register. Motors spin 
          motor[i-6][motorDeltaMoves] = 2;  // ensure we keep this loop active by keeping
          ballGoTo(i-5, red); }          // go directly to red. motors are addressed 1 thru 10, not 0-9                            
      }
      pinValues165 = read_shift_regs165();// read 165 shift registers again (the previous operation just destroyed the bit pattern???)
      
      Serial.println("");
      Serial.print(">");
      for(byte i = 6; i < DATA_WIDTH; i++){  //loop thru each bit from bit positions 6 thru 15 (== units 0 thru 9)
        Serial.print((pinValues165 >> i), BIN);
        if(((pinValues165 >> i) == false) && (motorRecalibration[i-6] == true)) {       // if the bit equal to 0 (false) and recalibration flag is true, then
                                            // we have just finished an unwind to position index 0
          motorRecalibration[i-6] = false;  // set this unit's recalibration flag to false                                            
          Serial.print(i-6);
          Serial.println(" 0:D");    // we are at position 0 and need to go to the ceiling
          motor[i-6][motorTargetIndex] = 0;
          motor[i-6][motorDeltaMoves] = 0; //motor[i][motorTargetIndex] - motor[i][motorCurrentIndex] - 1; //move this many steps: target - Current - 1
          setRegisterPin((3*(i-6)),LOW);           // disable motor bit in shift register                                 
          writeRegisters();                       // latch shift register. 
          ballFadeTo(i-5, blue);}             // motors are addressed 1 thru 10, not 0-9          
    }
...
for (int i=0 ; i < 10; i++) {                // for each motor 
  ...
  //Serial.println("Leaving moveBall");
}
//------------------------------------
...
void recalibrate() {       // Recalibrate
  ...
  ballFadeTo(allBlinkMs, yellow);
             for (int i=0 ; i < 10; i++) {   // assign value to all motor targetIndex
              motor[i][motorCurrentIndex] = 255;
              motor[i][motorTargetIndex] = 0;
             };
  
  moveTo();               // go as high as we can till we hit the IR detectors
  ...
  moveTo();
  //ballFadeTo(cyan);
  //Serial.println("Leaving Recalibrate");
  }
//------------------------------------
