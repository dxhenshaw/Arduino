...
//Constants:
  ...
  const int minPauseTime = 10;              // in seconds = 2 minutes = 120 xxx
  const int maxPauseTime = 20;              // in seconds = 5 minutes = 300 xxx
...
//Variables:
  ...
  int floorIndex = 40;                 //index for floor level xxx 60
...
if(digitalRead(encoderSwitchPin)){  // check if button has been pushed 
    switch (selectedMenuItem) {  // button is not being pushed, which means we can execute code based on it being pushed last time round this loop
      ...
      case 4:  // 
       ...
       do{
         ...
         curtainsOut(100, white, black);
         BlinkM_fadeToRandomRGB(allBlinkMs, 0xff, 0xff, 0xff);      // set a random color
         programPauseTime = random(5, 10);       // in seconds - wait before moving xxx
         pauseTime(programPauseTime); 
    ...
    if (autoRunFlag == true){  // we're in auto-run mode and need to cycle through each program in turn
      ...
      if (selectedMenuItem >= 7) selectedMenuItem = 1; } // next time round, the correct program will run xxx replace with constant
...
for (int i=0 ; i < 10; i++) {
  ...
  if (motor[i][motorCurrentIndex] > motor[i][motorTargetIndex]) {  // if currentIndex > targetIndex, need to move up CW
    Serial.print("Up ");
    motorDirectionUp(i);    
    motor[i][motorDeltaMoves] = motor[i][motorCurrentIndex] - motor[i][motorTargetIndex] + 0;  //move this many steps: current - Target + 0 xxx +0?
    ...
    motor[i][motorDeltaMoves] = motor[i][motorTargetIndex] - motor[i][motorCurrentIndex] ; //move this many steps: target - Current - 1 xxx -1?
  }
}
...
for (int i=0 ; i < 10; i++) {            // figure out which motors need to have their pin enabled
 ...
 do {     
    ...
    for (byte l=3; l>0; l--) {            // check ball status 3 times per second
      ...
      //for(byte i=0; i<10; i++){  
      //  Serial.print(switchValue[i], BIN); }
      //Serial.println("<");
...
if (motorStateChange == true) writeRegisters();                // latch shift register. Motors spin xxx  is this going to be called too much? 3 x second?
...
void recalibrate() {       // Recalibrate
  ...
  ballFadeTo(allBlinkMs, yellow);
             for (int i=0 ; i < 10; i++) {   // assign value to all motor targetIndex i < 10 xxx
