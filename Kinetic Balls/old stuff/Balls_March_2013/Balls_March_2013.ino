...
prog_char menuItem_17[] PROGMEM = "Moving...";
prog_char menuItem_18[] PROGMEM = "Moving down...";   //xxx not currently being used
...
int motor[10][3];  // 10 motors, 3 attributes
// currentIndex | targetIndex | deltaMoves
    ...
    switch (selectedMenuItem) {
      ...
      case 1:    // ladder program (up and down while running scripts)
        do {    // main ladder loop
          ...
          do { // head down
            targetIndex = programTargetIndex(motor[0][motorCurrentIndex], bottomIndex); // find a random place to go between currentIndex of motor 0 [0][0] and bottom
            for (int i=0 ; i < 10; i++) {   // assign value to all motor targetIndex
              motor[i][motorTargetIndex] = targetIndex;  // position [1] is targetIndex
              //Serial.print("Target for Motor ");
              //Serial.print(i);
              //Serial.print("=");
          ...
          } while (motor[0][motorCurrentIndex] != bottomIndex);   //[0][0] = motor 0, currentIndex value
         if (abs(lastEncoderValue - encoderValue) > encoderGap) break;
                   Serial.println("@pgm 1 up");
         do { // head up
            targetIndex = programTargetIndex(ceilingIndex, motor[0][motorCurrentIndex]); // find a random place to go between currentIndex of motor 0 [0][0] and top
            for (int i=0 ; i < 10; i++) {   // assign value to all motor targetIndex
      ...
      case 2:   // random points program
        ...
        do {
           for (int i=0 ; i < 10; i++) { 
             motor[i][motorTargetIndex] = programTargetIndex(ceilingIndex, bottomIndex);             // find a random place to go for each ball
           }
          programPauseTime = random(minPauseTime, maxPauseTime);           // in seconds - wait before moving
          pauseTime(programPauseTime);                                // convert to milliseconds xxx add message on pause time
...
for (int i=0 ; i < 10; i++) {
  ...
  if (motor[i][motorCurrentIndex] > motor[i][motorTargetIndex]) {  // if currentIndex > targetIndex, need to move up CW
    Serial.print("Up ");
    setRegisterPin((3*i)+1,HIGH); // set appropriate 595 register pin
    setRegisterPin((3*i)+2,LOW);    
    ...
    Serial.print("Down ");
    setRegisterPin((3*i)+1,LOW); // set appropriate 595 register pin
    setRegisterPin((3*i)+2,HIGH);
...
for (int i=0 ; i < 10; i++) {  
 if (recalibrationFlag == false) {// xxx fix for per-ball
   motor[i][motorCurrentIndex] = motor[i][motorTargetIndex];}
 else {
   motor[i][motorCurrentIndex] = 0; }
}
...
for (int j = maximumDelta ; j >= 0; j--) { // get ready to do j number of 1-second loops
  lcd.setCursor(17,3);
  ...
  for (byte l=10; l>0; l--) {            // check ball status multiple times per second
     ...
     if ((pinValues165 >> 0) & 1){       // we are at recalibration point
       ...
       Serial.println(pinValues165, BIN);
...
void recalibrate() {       // Recalibrate
  ...
  moveTo(); //xxx adjust to move to ceiling index
  //ballFadeTo(cyan);
  }
//------------------------------------
