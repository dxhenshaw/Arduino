...
prog_char menuItem_10[] PROGMEM  = "LIGHTS TEST";
...
//Constants:
  ...
  const byte maxCycles = 15;            // How many loops to run through before an automatic recalibration  xxx normally 10 or more?  
  ...
  const byte maxAutoRunPrograms = 13; // when on auto run, when to loop back to program #1 (use value of sleep mode) xxx revise this so sun and moon isn't included
...
//Variables:
  ...
  byte wakeHour;                 // time to wake up xxx need to get into eeprom
  byte loopCounter = 0; 
    ...
    switch (selectedMenuItem) {  // button is not being pushed, which means we can execute code based on it being pushed last time round this loop
      ...
      case 4:  // 
       ...
       do{
         ...
         curtainsOut(100, white, black);
         BlinkM_fadeToRandomRGB(allBlinkMs, 0xff, 0xff, 0xff);      // set a random color
         programPauseTime = random(minPauseTime, maxPauseTime);       // in seconds - wait before moving xxx
         pauseTime(programPauseTime); 
      ...
      case 10:  // "Light test"      
      do {
        //randomLightShow(10); // force custom script
        //delay(5000);
        
        //randomLightShow(11); // force custom script
        //delay(1000);
        ...
        do{ // let's set wake hour...
            if (abs(lastEncoderValue - encoderValue) > encoderGap){
              if (encoderValue > lastEncoderValue) {
                ...
                if (wakeHour == sleepHour) wakeHour = addHour(wakeHour);
            }else{ 
                wakeHour = subtractHour(wakeHour);
                if (wakeHour == sleepHour) wakeHour = subtractHour(wakeHour);
            }
            }          
          lastEncoderValue = encoderValue;
          delay(100);
      ...
      case 19:  //Set floor xxx needs to log to eeprom
        recalibrate();  //first, recalibrate
        printStatusMessage(msgUseDial);
