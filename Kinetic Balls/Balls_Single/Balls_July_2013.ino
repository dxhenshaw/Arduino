/*
 * Kinetic Balls -- David Henshaw, February 2014
 * Modified to work for just one ball for dorkbot-sf presentation
 *
...
#define number_of_74hc595s 1						// How many shift registers
...
void setup()
{
  ...
  Serial.begin(115200);						// start serial comms for terminal19200
...
//Configure pins:
  ...
  randomSeed(analogRead(A1));				// random seed based on empty analog pin 1
	...
	ceilingIndex =  3; //EEPROM.read( ceilingIndex_EEPROM );              
	floorIndex =   43; //EEPROM.read( floorIndex_EEPROM ); 
...
void loop() {
	...
	playScript(scriptNumber);

	pinValues165 = read_shift_regs165();		// read 165 shift registers to detect microswitch
	if((pinValues165 >> (0 + 6)) & true){	// Switch has closed (need to adjust by 6 because of circuit design)
		ballFadeTo( allBlinkMs, red);
		motor[0][motorCurrentIndex] = 10; 
		motor[0][motorTargetIndex] = 20;
		moveTo();
		  } else {
